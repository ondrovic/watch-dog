...
- What happens when multiple parents of the same dependent become unhealthy in a short window? Each parent is restarted and waited on until healthy. To avoid restarting the same dependent multiple times, the monitor MAY apply a **dependent restart cooldown**: it skips restarting a dependent if that dependent was already restarted within a configurable time window (e.g. 90 seconds). Thus, at most one restart per dependent per cooldown window is applied when multiple of its parents recover in quick succession. This mitigation keeps behavior correct while reducing redundant restarts (e.g. for containers with many parents).
...
- **FR-006**: The system MUST support multiple parents (a container may depend on more than one parent); when a parent becomes unhealthy, the system restarts that parent, waits for it to be healthy, then restarts all containers that list that parent in depends_on. To mitigate redundant restarts when multiple parents of the same dependent recover in a short window, the system MAY skip restarting a dependent if it was restarted within a configurable cooldown (at most one restart per dependent per cooldown window).
