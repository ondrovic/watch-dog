...
- Q: Should healthcheck interval and related parameters be hardcoded or sourced from .env? → A: Pull from .env; use canonical variable names with the DOCKER_HEALTHCHECK_ prefix (DOCKER_HEALTHCHECK_INTERVAL, DOCKER_HEALTHCHECK_RETRIES, DOCKER_HEALTHCHECK_START_PERIOD, DOCKER_HEALTHCHECK_TIMEOUT); reference values: 15s, 2, 20s, 10s. Documented configuration must not hardcode values.
...
- Q: Which log output format styles should be supported and how are they selected? → A: Log format is configurable (e.g. via LOG_FORMAT); at least two documented styles: "compact" ([Level] message) and "timestamp" (timestamp [Level] message). A third style "json" is recommended for machine parsing. Custom names are defined and documented; default when unset is "timestamp". Supported styles are "compact", "timestamp", and "json".
...
- **FR-002**: The healthcheck MUST report healthy when the monitor process is running and able to communicate with the Docker API. The healthcheck MUST use a concrete, lightweight Docker command (e.g. `docker info` or `docker ps --format '{{.ID}}'`); no HTTP endpoint is required. The healthcheck command MUST complete in under 1s under normal load and MUST NOT require elevated permissions beyond the watch-dog process's normal Docker socket access.
...
- **FR-004**: The healthcheck MUST run at a defined interval with a bounded start period, timeout, and retries. Parameters MUST be configurable via .env using the canonical variable names DOCKER_HEALTHCHECK_INTERVAL, DOCKER_HEALTHCHECK_RETRIES, DOCKER_HEALTHCHECK_START_PERIOD, DOCKER_HEALTHCHECK_TIMEOUT; documented configuration (e.g. compose example) MUST use these variables rather than hardcoded values. Reference values for documentation: 15s, 2, 20s, 10s respectively.
- **FR-005**: The healthcheck definition and parameters MUST be documented in the README (and in a compose example where relevant) using the .env variable names (DOCKER_HEALTHCHECK_INTERVAL, DOCKER_HEALTHCHECK_RETRIES, DOCKER_HEALTHCHECK_START_PERIOD, DOCKER_HEALTHCHECK_TIMEOUT) so that values are not hardcoded; users and operators know how health is determined, how to interpret healthy/unhealthy, and how to override via .env if needed. *(FR-004 defines the healthcheck behavior and .env parameter names; FR-005 defines the documentation and compose-example obligation.)*
...
- **Log format (LOG_FORMAT)**: Environment variable selecting output style. Supported styles: **compact** – `[Level] message`; **timestamp** – `timestamp [Level] message`; **json** – one JSON object per line. Default when unset: **timestamp**.
