...
- **Action**: The callback performs the equivalent of `docker compose -f <composePath> up -d <service_name>` using the **Docker Compose Go SDK** (in-process). No `docker` or `docker-compose` binary is required. The monitor loads the compose project (config path, working directory, optional project name from `COMPOSE_PROJECT_NAME`) and calls the SDK’s `Up` with the single service and force-recreate options. The project is loaded with environment variables from a `.env` file: by default from the compose file’s directory, or from the path given by `WATCHDOG_ENV_FILE` when set. Variables used in the compose file (e.g. healthcheck `retries`, `interval`) must be defined in that `.env` or in the process environment or interpolation may fail. Working directory is the compose file’s directory (or current directory when appropriate). The monitor resolves the parent's **container name** (e.g. `vpn`) to the compose **service name** (e.g. `gluetun`) using the compose file's `container_name` when set, so the correct service is recreated. The monitor does not wait for the operation to finish; it continues and will re-discover on the next cycle.
...
- **Observability**: When auto-recreate is triggered, the monitor logs at INFO the parent name, service name (if resolved), compose path, and that it will re-discover on the next cycle. On success it logs that auto-recreate (Compose SDK) succeeded. If the SDK init (at startup) or the Up call fails, an error is logged.
