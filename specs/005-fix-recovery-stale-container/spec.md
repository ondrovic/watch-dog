...
- **FR-004**: The system MUST allow the same logical service to be recovered later (e.g. after containers are re-discovered or recreated), so that stopping retries for one container identity does not permanently prevent recovery of that service.
...
- **SC-003**: After the monitor has stopped retrying a failed recovery, if the same logical service is recreated or becomes available again, the monitor can resume normal monitoring and recovery for that service within one poll interval or on the next Docker event, whichever comes first.
